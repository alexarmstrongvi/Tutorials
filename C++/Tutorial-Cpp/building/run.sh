...
################################################################################
# NOTES
# * MacOS offers the `clang` command line tool (a.k.a. driver) for building C/C++ source code.
#
# * MacOS also seems to offer `gcc` builtin but this really just runs clang. The real GNU gcc compiler must be installed.
#
# * `clang` is a driver that drives the build process, calling the compiled (also called `clang` but a different executable) and linker (`ld`). Use the verbose option (`-v`) to see all the commands run by the driver.
#
